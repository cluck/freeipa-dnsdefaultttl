...
PLUGIN = 'dnsdefaultttl'
PACKAGES = ['ipalib_dnsdefaultttl']

with codecs.open(os.path.join('.', 'ipalib_dnsdefaultttl', 'version.py')) as init:
    METADATA = dict(re.findall("__([A-Za-z][A-Za-z0-9_]+)__\s*=\s*'([^']+)'", init.read().decode('utf-8')))

if os.path.exists("README.md"):
    long_description = codecs.open("README.md", "r", "utf-8").read()
else:
    long_description = "See https://github.com/cluck/ipalib-dnsdefaultttl"
...
class post_install(install_data):
	...
	shutil.copy(
            os.path.join(self.install_dir, 'ipalib/plugins/dnsdefaultttl.py'),
            os.path.join(IPALIB_DIR, 'plugins/dnsdefaultttl.py'),
        )

extra = {}

setup(
    name = 'ipalib-dnsdefaultttl',
    packages=PACKAGES,
    version=METADATA['version'],
    description = 'DNS Default TTL plugin for FreeIPA 4',
    long_description = long_description,
    url = 'https://github.com/cluck/ipalib-dnsdefaultttl',
    ...
    entry_points={
        'console_scripts': [
            'dnsdefaultttl = ipalib_dnsdefaultttl.commands:dnsdefaultttl',
        ],
    },
    zip_safe=False,
    classifiers=[
